...
module HcaService
  ...
  def authorize_url(redirect_uri, state, login_hint: nil)
    params = {
      client_id: ENV.fetch("HCA_CLIENT_ID", nil) || ENV.fetch("HACKCLUB_CLIENT_ID", nil),
  ...
  def exchange_code_for_token(code, redirect_uri)
    response = connection.post("/oauth/token") do |req|
      ...
      req.body = {
        ...
        redirect_uri: redirect_uri,
        client_id: ENV.fetch("HCA_CLIENT_ID", nil) || ENV.fetch("HACKCLUB_CLIENT_ID", nil),
        client_secret: ENV.fetch("HCA_CLIENT_SECRET", nil) || ENV.fetch("HACKCLUB_CLIENT_SECRET", nil)
      }.to_query
    end
