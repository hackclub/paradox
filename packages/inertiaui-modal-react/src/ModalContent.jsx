  ...
  return (
    <div className="im-modal-container fixed inset-0 z-40 overflow-y-auto p-4">
      ...
      >
        <TransitionChild
          as="div"
          enterFrom={config.duration === 0 ? '' : 'opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95'}
          enterTo="opacity-100 translate-y-0 sm:scale-100"
          leaveFrom="opacity-100 translate-y-0 sm:scale-100"
          leaveTo={config.duration === 0 ? '' : 'opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95'}
          afterEnter={() => setEntered(true)}
          afterLeave={modalContext.afterLeave}
          className={clsx(
            'im-modal-wrapper w-full transition duration-300 ease-in-out',
            config.duration !== 300 && scopeClass,
